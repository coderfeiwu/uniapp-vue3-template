<template>
  <view>
    <scroll-view
      ...
      class="box mt-[300rpx]"
    >
      <view v-for="item in blueDeviceList" :key="item.deviceId" class="item" @click="connect(item)">
        <view>
...
// 【3】找到新设备就触发该方法
function found(res) {
  if (res.devices[0].RSSI > 90 && res.devices[0].name && res.devices[0].name?.includes('GTS7 PRO')) {
...
const serviceId = ref('6E400001-B5A3-F393-E0A9-E50E24DCCA9D');
...
function hexCharCodeToStr(hexCharCodeStr) {
  ...
  for (let i = 0; i < len; i = i + 2) {
    curCharCode = Number.parseInt(rawStr.substr(i, 2), 16);
